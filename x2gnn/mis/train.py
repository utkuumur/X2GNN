...
def train_iteration(model, train_dataloader, optimizer, scheduler, device, R, p, DC, k):
    ...
    for data in train_dataloader:
        ...
        for lp in layer_probs:
            mis_loss, penalty_loss = compute_mis_loss(data, lp)        
            ...
            loss += -mis_loss + penalty_loss + DC * div_loss
        ...
        objs, objs2 = compute_obj_mis_refine(data, layer_probs[-1], k)
...
def eval_iteration(model, valid_dataloader, optimizer, scheduler, device, R, p, DC, k):
    ...
    with torch.no_grad():
        for data in valid_dataloader:
            ...
            mis_loss, penalty_loss = compute_mis_loss(data, layer_probs)
            ent_loss = compute_loss_entropy(data, layer_probs)
            div_loss = compute_diversity_loss(data, layer_probs, k)
        
            # Accumulate losses
            val_loss += (-mis_loss + penalty_loss)
            val_mis_loss -= mis_loss
            val_pen_loss += penalty_loss
            val_ent_loss += ent_loss
            val_div_loss += div_loss
            
            # Compute objective metrics
            objs, pens = compute_mis_obj(data, layer_probs)
            objs2, obsj2best = compute_obj_mis_refine(data, layer_probs, k)
            
            valid_objectives.append(objs)
            valid_objectives2.extend(objs2)
            valid_objectives3.extend(obsj2best)
            valid_pens.append(pens)
            
    # Calculate average metrics
    z = (len(valid_dataloader) + 1)
    valid_objectives = torch.mean(torch.stack(valid_objectives)).item()
    valid_objectives2 = np.mean(valid_objectives2)
    valid_objectives3 = np.mean(valid_objectives3)
    valid_pens = torch.mean(torch.stack(valid_pens)).item()
    
    return valid_objectives, valid_objectives2, valid_objectives3, val_loss.item() / z, val_mis_loss.item() / z, val_pen_loss.item() / z, val_ent_loss.item() / z, valid_pens, val_div_loss.item() / z 
...
def main(args):
    ...
    for epoch in tqdm(range(args.ep)):
        train_obj, train_obj2 = train_iteration(model, train_dataloader, optimizer, scheduler, device, 1, 0, args.div_coef, args.k)
        valid_obj, valid_obj2, valid_obj3, loss, loss_mis, loss_pen, loss_ent, valid_pen, loss_div = eval_iteration(
            model, valid_dataloader, optimizer, scheduler, device, 1, 0, args.div_coef, args.k
        )
            
        # Scale losses by k parameter
        loss, loss_mis, loss_pen, loss_ent, valid_pen = loss / args.k, loss_mis / args.k, loss_pen / args.k, loss_ent / args.k, valid_pen / args.k
        
        # Log training progress
        logger.info(
            f'Epoch: {epoch} TrainMeanObjB={train_obj2:.1f} EvalMeanObjB={valid_obj3:.3f} '
            f'TrainMeanObjM={train_obj:.1f} EvalMeanObjM={valid_obj2:.3f} EvalMeanObj={valid_obj:.1f} '
            f'EvalMeanPen={valid_pen:.1f} Loss={loss:.4f} MISLoss={loss_mis:.4f} '
            f'PenLoss={loss_pen:.4f} EntLoss={loss_ent:.4f} DivLoss={loss_div:.4f}'
        )
            
        # Log to wandb if enabled
        if args.wandb: 
            wandb.log({
                'Validation Mean Objective': valid_obj, 
        ...
        if args.save:
            sp = f'../../models/mis/{model_identifier}'   
            if args.model_path != '':
                sp = args.model_path
    ...
    for epoch in tqdm(range(args.ep)):
        ...
        train_obj, train_obj2 = train_iteration(model, train_dataloader, optimizer, scheduler, device, R, p, args.div_coef, args.k)
        valid_obj, valid_obj2, valid_obj3, loss, loss_mis, loss_pen, loss_ent, valid_pen, loss_div = eval_iteration(
            model, valid_dataloader, optimizer, scheduler, device, R, p, args.div_coef, args.k
        )
        ...
        # Log training progress
        logger.info(
            f'Epoch: {epoch} R={R} p={p:.2f} TrainMeanObjB={train_obj2:.1f} EvalMeanObjB={valid_obj3:.3f} '
            f'TrainMeanObjM={train_obj:.1f} EvalMeanObjM={valid_obj2:.3f} EvalMeanObj={valid_obj:.1f} '
            f'EvalMeanPen={valid_pen:.1f} Loss={loss:.4f} MISLoss={loss_mis:.4f} '
            f'PenLoss={loss_pen:.4f} EntLoss={loss_ent:.4f} DivLoss={loss_div:.4f}'
        )
        ...
        if args.wandb: 
            wandb.log({
                'Validation Mean Objective': valid_obj, 
                'Validation Mean Objective Refined': valid_obj3, 
                'Training Mean Objective Refined': train_obj2, 
                'MIS Loss': loss_mis, 
                'Pen Loss': loss_pen, 
                'Ent Loss': loss_ent
            })
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Maximum Independent Set (MIS) solver using neural networks')
    # Training
    ...
    parser.add_argument('--wandb', default=False, action='store_true', help='Enable logging and tracking with Weights & Biases (wandb)')
    parser.add_argument('--compile', default=False, action='store_true', help='Compile the model using torch.compile for optimized performance if available')
